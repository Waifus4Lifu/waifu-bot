...
async def rate_limiter(message):
    ...
    if len(attachments) > 5:
        ...
        description = (
            description + "You're not breaking the rules, but you are being a scumbag."
        )
        embed = discord.Embed(title=title, description=description, color=waifu_pink)
        await message.channel.send(embed=embed)
    return
        ...
        if message.author not in general_chat.members:
            for channel in get_guild().text_channels:
                if "quarantine" in channel.name and message.author in channel.members:
                    reply = (
                        f"Hey everyone, {message.author.mention} just joined. {message.author.mention}, please "
                    ...
                    return
        reply = (
            f"Hey everyone, {message.author.mention} just joined. {message.author.mention}, please introduce "
            f"yourself. Thanks! "
        )
        await general_chat.send(reply)
        return
...
async def monitor_deletions():
    ...
    while True:
        ...
        if "deleted" in message.channel.name:
            super_waifu_chat = get_channel("super_waifu_chat")
            reply = (
                f"Hey {deleted_by.mention}, it's best if you don't delete messages from {message.channel.mention}."
...
async def monitor_joins():
    ...
    while True:
        ...
        block_noobs = True
        noob_channel = await guild.create_text_channel(
            "welcome_noob", topic=str(member.id), overwrites=overwrites
        )
        await asyncio.sleep(1)
        ...
        if invite_found is not None:
            ...
            if (
                invite_found.max_uses == 2
                and invite_found.uses == 1
                and invite_details is not None
            ):
                event_name = None
        ...
        reply = (
            f"Hey {member.mention}, welcome to Waifus_4_Lifu! I'm WaifuBot, I manage various things here."
            f" Here is a basic outline of our rules: "
        )
        await noob_channel.send(reply)
        reply = (
            "1. Don't be a dick. We all like to have fun and mess around but let's try and keep it playful! On "
...
async def on_message(message):
    if message.author.name == "among_us":
            ...
            for game_channel, game in among_us_games.items():
                for player, color in game.players.items():
                    if host == player:
                        ...
                        break
            if channel is None:
                log.error(f"Game host not in any games: {user_id} - {user_name}")
                return
        ...
        if command == "mute":
            ...
            for player, color in game.players.items():
                if color in dead:
                    await player.edit(
                        voice_channel=get_channel("Among The Dead"), mute=False
                    )
    ...
    if isinstance(message.channel, discord.TextChannel):
        if message.channel.name == "welcome_noob" and message.channel.topic == str(
            message.author.id
        ):
...
async def members(ctx, *, role):
    ...
    if role.name in config["roles"]["forbidden"] or role.color not in role_colors:
        reply = (
            f"{ctx.author.mention}, this is a Forbidden Role:tm:. If you can't figure this out a different way, "
...
async def _random(ctx):
    ...
    def check(answer):
        if answer.channel == ctx.channel:
            ...
            for word in is_not:
                if (
                    word in answer.content.lower()
                    and "random" in answer.content.lower()
...
@commands.guild_only()
async def sponge(
    ctx, target: typing.Optional[typing.Union[discord.Member, discord.Message, str]]
):
    ...
    if not isinstance(target, discord.Message):
        reply = (
            f"{ctx.author.mention}, I don't see any matching messages in this channel."
        )
        await ctx.send(reply)
        return
...
async def quoth(
    ...
    if ctx.channel != target.channel:
        ...
        await ctx.send(reply)
        reply = (
            f"{ctx.author.mention} tried to cross-quoth. Pretty dickish if you ask me."
        )
...
async def shake(
    ...
    for attachment in attachments:
        ...
        if image == "format":
            reply = (
                f"{ctx.author.mention}, attachment '{attachment.filename}' isn't in a valid format."
        ...
        if actual_size > 8000000:
            reply = (
                f"{ctx.author.mention}, attachment '{attachment.filename}' is too big."
            )
            await ctx.send(reply)
            image.close()
            file.close()
...
async def createrole(ctx, role, *, description: typing.Optional[str]):
    ...
    description = (
        f"{role_type.capitalize()}: {role.mention}\nCreated by: {ctx.author.mention}\n"
    )
...
async def editrole(ctx, role: discord.Role, *, description: typing.Optional[str]):
    ...
    if role.name in config["roles"]["forbidden"] or role.color not in [
        discord.Color.orange(),
        discord.Color.blue(),
    ]:
...
async def deleterole(ctx, role: discord.Role):
    ...
    description = (
        f"{role_type.capitalize()}: {role.name}\nDeleted by: {ctx.author.mention}\n"
    )
...
async def invite(ctx, *, reason: typing.Union[discord.CategoryChannel, str]):
    ...
    if event is None or event_role is None:
        ...
        invite = await channel.create_invite(
            max_age=86400, max_uses=2, temporary=False, unique=True, reason=reason
        )
    else:
        type = "EVENT"
        reason = None
        invite = await channel.create_invite(
            max_uses=100, temporary=False, unique=True, reason=reason
        )
...
@commands.guild_only()
async def activity(
    ctx, member: typing.Optional[discord.Member], days: typing.Optional[int]
):
    ...
    if member is None:
        ...
        members = [member]
    await ctx.send(
        f"Preparing activity report for {len(members)} member(s). I will post when the results are ready."
    )
    ...
    for channel in guild.text_channels:
        async for message in channel.history(
            limit=None, after=search_start, before=search_end
        ):
    ...
    for member in members:
        ...
        value += (
            f"{member.mention} Text: {text_count} Voice: {format_delta(voice_delta)}\n"
        )
    ...
    description = (
        f"Search start: {formatted_start}\n"
    ...
    embed.set_footer(
        text=f"*Voice activity based on data since the bot started watching: {timestamp}"
    )
    await ctx.send(embed=embed)
    return
    ...
    if len(members) < 2:
        ...
        for i in range(100):
            ...
            if not mismatch:
                for i in range(len(members)):
                    message = (
                        f"{senpai[i].display_name}, your covert kohai is {kohai[i].display_name}"
                    ...
                    try:
                        ...
                        warning = (
                            f"{senpai[i]} won't accept DM's. SPOILER: ||'{message}'||"
                        )
...
async def createevent(
    ctx,
    event: typing.Union[discord.CategoryChannel, str],
    *,
    YYYYMMDDHHMMSS: typing.Optional[str],
):
    ...
    event_role = await guild.create_role(
        name=name.lower(), mentionable=True, color=discord.Color.orange()
    )
    countdown = {
        noob_role: discord.PermissionOverwrite(
    ...
    general = {
        ...
        quarantine_role: discord.PermissionOverwrite(
            read_messages=False, send_messages=False, connect=False
        ),
    ...
    if YYYYMMDDHHMMSS is not None:
        ...
        await category.create_text_channel(
            name="countdown", overwrites=countdown, topic=topic
        )
    primary_channel = await category.create_text_channel(
        name=f"{name.lower()}_chat", overwrites=general
    )
    await category.create_voice_channel(
        name=f"{name.lower()}_voice", overwrites=general
    )
    await category.create_text_channel(
        name="questions", overwrites=general, slowmode_delay=300
    )
    await category.create_text_channel(name="looking_for_room", overwrites=general)
    await category.create_text_channel(
        name="quarantine_chat", overwrites=quarantine, slowmode_delay=10
    )
    ...
    embed = discord.Embed(title=title, description=description, color=waifu_pink)
    value = (
        f"!invite {category.name} - Create 100 use event invite.\n"
...
async def deleteevent(ctx, *, event: typing.Union[discord.CategoryChannel, str]):
    ...
    if (
        ...
        or event_name in config["roles"]["forbidden"]
    ):
        reply = f"{ctx.author.mention}, that is not a valid event/role."
        await ctx.send(reply)
        return
...
async def image(ctx, *, query):
    ...
    url = (
        "https://www.googleapis.com/customsearch/v1?cx=012763604623577894851:r8w2tzy60qx"
